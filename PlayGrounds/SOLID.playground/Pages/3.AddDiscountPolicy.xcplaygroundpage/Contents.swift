...
class RateDiscountPolicy: DiscountPolicy {
    private let discountPercent = 10
...
class DiscountTests: XCTestCase {
    var rateDiscountPolicy: RateDiscountPolicy!
    ...
    override func tearDown() {
        super.tearDown()
        rateDiscountPolicy = nil
    }

    // vip가 10% 할인 적용되는지 확인
    func testVipDiscount() {
        // given
        let member = Member(name: "memberA", grade: .vip)

        // when
        let discount = rateDiscountPolicy.discount(member: member, price: 10000)
        
        // then
        XCTAssertEqual(discount, 1000)
    }
    
    // vip가 아닌경우 0 원이 할인되는지 확인
    func testNotVip() {
        // given
        let member = Member(name: "memberBASIC", grade: .basic)

        // when
        let discount = rateDiscountPolicy.discount(member: member, price: 10000)
        
        // then
        XCTAssertEqual(discount, 1000) //0 이 나와야 함
    }
}

DiscountTests.defaultTestSuite.run()
